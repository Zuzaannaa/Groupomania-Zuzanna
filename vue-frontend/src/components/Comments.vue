    ...
    <div :key="comment.id" v-for="comment in comments" class="comment">
      <div class="header">
        <div class="profileContainer">
          <img
            :src="'http://localhost:3000/images/' + comment.User.profile"
        ...
        <div v-if="auth(comment.User.id)" class="optionsBtn">
          ...
          >
            <i class="far fa-edit modify"></i>Edit
          </button>
          <button
            v-if="editComment == comment.id"
            @click="toggleComment(comment.id)"
          ...
          <button @click="deleteComment(comment.id)">
            <i class="far fa-trash-alt delete"></i>Delete
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    deleteComment(commentId) {
      if (
        confirm("Comment will be deleted. Are you sure you want to proceed?")
      ) {
        fetch(
