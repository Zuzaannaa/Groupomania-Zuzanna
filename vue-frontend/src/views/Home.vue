<template>
  ...
  <Header :home="true" header="Welcome" />
  <button v-if="createPost" @click="toggleCreate" class="btn">
    Back to posts
  </button>
  <button v-else @click="toggleCreate" class="btn">Create a post</button>
  <CreatePost
    v-show="createPost"
    @toggle-Create="toggleCreate"
    @add-Post="addPostFrontend"
  />
  <div v-if="!createPost" id="query">
    <span><i class="fas fa-search"></i></span>
    <input type="text" placeholder="find a user" v-model="search" />
  </div>
  <Posts
    :posts="filterPosts"
    @delete-Post="deletePostFrontend"
    :isAdmin="isAdmin"
    :userId="userId"
  />
</template>
