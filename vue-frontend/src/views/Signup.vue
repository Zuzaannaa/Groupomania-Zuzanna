  ...
  methods: {
    onSubmit() {
      ...
      if (!data.firstname || !data.lastname || !data.email || !data.password) {
        this.errMsg = "No empty fields allowed";
        return;
      }
      ...
      if (!regexName.test(data.firstname && data.lastname)) {
        this.errMsg = "Name format incorrect";
        return;
      }
      if (!regexEmail.test(data.email)) {
        this.errMsg = "Email error";
        return;
      }
      ...
      fetch("http://localhost:3000/api/users/signup", {
        ...
        .then((res) => {
          if (res.status === 201) {
            fetch("http://localhost:3000/api/users/login", {
              ...
              .then((res) => {
                ...
                if (res.status === 200) {
                  ...
                  this.errMsg = "Email address or password are incorrect";
                }
              })
              .catch((error) => {
                console.error(error);
            ...
            this.errMsg = "Email address already excists";
            return;
          }
        })
        .catch((error) => {
